...
public class StaffModel {
    ...
    public boolean addStaff(Staff staff, List<StaffFamily> familyMembers, StaffRoles staffRole, Positions position) {
        ...
        String insertStaffQuery = "INSERT INTO Staffs (FirstName, LastName, DateOfBirth, Gender, Address, PhoneNumber, Email, Password, HireDate, PositionID, Salary, EducationBackground, Experience, Avatar, Status, ResetCode, ResetCodeCreationTime, IsResetCodeUsed) " +
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'active', ?, ?, ?)";

        String insertStaffRoleQuery = "INSERT INTO StaffRoles (StaffID, RoleName) VALUES (?, ?)";
        String insertPositionQuery = "INSERT INTO Positions (PositionID, PositionName) VALUES (?, ?)";
        ...
        try (Connection conn = ConnectDB.connection();
             ...
             PreparedStatement positionStatement = conn.prepareStatement(insertPositionQuery);
            ...
            if (rowsAffected > 0) {
                try (ResultSet generatedKeys = staffStatement.getGeneratedKeys()) {
                    if (generatedKeys.next()) {
                        ...
                        // Xử lý bảng Positions
                        if (position != null) {
                            // Kiểm tra xem vị trí đã tồn tại hay chưa
                            String checkPositionExistsQuery = "SELECT COUNT(*) FROM Positions WHERE PositionID = ?";
                            try (PreparedStatement checkPositionStatement = conn.prepareStatement(checkPositionExistsQuery)) {
                                checkPositionStatement.setInt(1, position.getPositionID());
                                ResultSet rs = checkPositionStatement.executeQuery();
                                if (rs.next() && rs.getInt(1) == 0) {
                                    // Nếu vị trí chưa tồn tại, thêm mới
                                    positionStatement.setInt(1, position.getPositionID());
                                    positionStatement.setString(2, position.getPositionName());
    ...
    private boolean validateStaffData(Staff staff) {
        ...
        if (staff.getPositionID() <= 0) {
            System.out.println("ID vị trí phải lớn hơn 0.");
            isValid = false;
        }
    ...
    public boolean updateStaff(Staff staff) {
        ...
        try (Connection conn = ConnectDB.connection();
            ...
            if (staff.getResetCodeCreationTime() != null) {
                ...
                preparedStatement.setNull(17, java.sql.Types.TIMESTAMP);
            }
