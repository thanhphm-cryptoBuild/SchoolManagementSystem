...
public class StaffModel {
    ...
    public boolean addStaff(Staff staff, List<StaffFamily> familyMembers, StaffRoles staffRole) {
        ...
        String insertStaffQuery = "INSERT INTO Staff (FirstName, LastName, DateOfBirth, Gender, Address, PhoneNumber, Email, Password, HireDate, PositionID, Salary, EducationBackground, Experience, Avatar, Status, ResetCode, ResetCodeCreationTime, IsResetCodeUsed) " +
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'active', ?, ?, ?)";
        ...
        try (Connection conn = ConnectDB.connection();
            ...
            staffStatement.setInt(10, staff.getPositionID());
    ...
    public int getPositionIdOrAddPosition(String positionName) {
        // Kiểm tra xem vị trí đã tồn tại chưa
        String selectPositionQuery = "SELECT PositionID FROM Positions WHERE PositionName = ?";
        String insertPositionQuery = "INSERT INTO Positions (PositionName) VALUES (?)";

        try (Connection conn = ConnectDB.connection();
             PreparedStatement selectStatement = conn.prepareStatement(selectPositionQuery);
             PreparedStatement insertStatement = conn.prepareStatement(insertPositionQuery, PreparedStatement.RETURN_GENERATED_KEYS)) {

            // Kiểm tra xem vị trí đã tồn tại chưa
            selectStatement.setString(1, positionName);
            ResultSet rs = selectStatement.executeQuery();

            if (rs.next()) {
                // Vị trí đã tồn tại, trả về PositionID
                return rs.getInt("PositionID");
            } else {
                // Thêm vị trí mới và lấy PositionID
                insertStatement.setString(1, positionName);
                int affectedRows = insertStatement.executeUpdate();
                if (affectedRows > 0) {
                    try (ResultSet generatedKeys = insertStatement.getGeneratedKeys()) {
                        if (generatedKeys.next()) {
                            return generatedKeys.getInt(1); // Trả về PositionID mới tạo
                        }
                    }
                }
            }
        ...
        return -1; // Nếu có lỗi, trả về giá trị không hợp lệ
    }
    ...
    public boolean updateStaff(Staff staff, List<StaffFamily> familyMembers, StaffRoles staffRole) {
        ...
        String updateStaffQuery = "UPDATE Staff SET FirstName = ?, LastName = ?, DateOfBirth = ?, Gender = ?, Address = ?, PhoneNumber = ?, Email = ?, Password = ?, HireDate = ?, PositionID = ?, Salary = ?, EducationBackground = ?, Experience = ?, Avatar = ?, Status = 'active', ResetCode = ?, ResetCodeCreationTime = ?, IsResetCodeUsed = ? WHERE StaffID = ?";
    ...
    public List<Staff> searchStaff(String firstName, String email, Byte gender, Integer id) {
        List<Staff> staffList = new ArrayList<>();
        StringBuilder query = new StringBuilder("SELECT StaffID, FirstName, LastName, DateOfBirth, Gender, Address, PhoneNumber, Email, Password, HireDate, PositionID, Salary, EducationBackground, Experience, Avatar, Status, ResetCode, ResetCodeCreationTime, IsResetCodeUsed FROM Staff WHERE Status = 'active'");
